...
const AlbumManager = ({ isOpen, onClose, initialAlbumId }: AlbumManagerProps) => {
  ...
  const handleCreateAlbum = async () => {
    if (!newAlbumName.trim()) { toast.error('Entre un nom pour l\'album'); return; }
    await createAlbum.mutateAsync({ name: newAlbumName.trim(), description: newAlbumDescription.trim() || undefined });
    setNewAlbumName('');
    setNewAlbumDescription('');
    setView('list');
  };
  ...
  const handleDeleteAlbum = async (albumId: string) => {
    ...
    if (selectedAlbumId === albumId) { setSelectedAlbumId(null); setView('list'); }
  };

  const handleDeleteMedia = async () => {
    if (!deleteMediaConfirm) return;
    await removeMedia.mutateAsync({ albumId: deleteMediaConfirm.albumId, mediaId: deleteMediaConfirm.mediaId });
  ...
  return (
    <>
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  <DropdownMenuItem onClick={() => fileInputRef.current?.click()}>
                    <ImagePlus className="w-4 h-4 mr-2" /> Ajouter des médias
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => setShowShares(selectedAlbumId)}>
                    <Users className="w-4 h-4 mr-2" /> Voir les partages
                  </DropdownMenuItem>
                  <DropdownMenuItem className="text-destructive" onClick={() => setDeleteConfirm(selectedAlbumId)}>
                    <Trash2 className="w-4 h-4 mr-2" /> Supprimer l'album
                  </DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
            )}
              ...
              <AnimatePresence mode="wait">
                {view === 'list' && (
                  <motion.div key="list" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                    <AlbumListView albums={albums} isLoading={isLoading} useAlbumMedia={useAlbumMedia} useAlbumShares={useAlbumShares}
                      onSelect={(id) => { setSelectedAlbumId(id); setView('detail'); }} onCreate={() => setView('create')} />
                  </motion.div>
                )}
                {view === 'create' && (
                  <motion.div key="create" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                    <CreateAlbumView name={newAlbumName} description={newAlbumDescription}
                      onNameChange={setNewAlbumName} onDescChange={setNewAlbumDescription}
                      onSubmit={handleCreateAlbum} isPending={createAlbum.isPending} />
                  </motion.div>
                )}
                {view === 'detail' && selectedAlbumId && (
                  <motion.div key="detail" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0 }}>
                    <AlbumDetailView albumId={selectedAlbumId} useAlbumMedia={useAlbumMedia}
      ...
      <AlertDialog open={!!deleteConfirm} onOpenChange={() => setDeleteConfirm(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>Cette action est irréversible. Tous les médias et partages seront supprimés.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Annuler</AlertDialogCancel>
            <AlertDialogAction onClick={() => deleteConfirm && handleDeleteAlbum(deleteConfirm)}
      ...
      {showShares && (
        <SharesDialog albumId={showShares} isOpen={!!showShares} onClose={() => setShowShares(null)}
          useAlbumShares={useAlbumShares} stopSharing={stopSharing} />
      )}
      ...
      <AlbumGalleryViewer
        ...
        onDelete={(mediaId) => {
          if (selectedAlbumId) setDeleteMediaConfirm({ albumId: selectedAlbumId, mediaId });
        }}
      />

      <UploadProgressOverlay isVisible={isUploading} uploads={uploadProgress}
        totalFiles={uploadProgress.length} completedFiles={uploadProgress.filter(u => u.status === 'complete').length} />
    </>
  );
};
...
const AlbumListView = ({
  ...
  albums: any[]; isLoading: boolean; useAlbumMedia: (id: string) => any;
  useAlbumShares: (id: string) => any; onSelect: (id: string) => void; onCreate: () => void;
}) => {
  ...
  if (albums.length === 0) {
    return (
      <div className="text-center py-16">
        ...
        <Button onClick={onCreate} className="rounded-xl gap-2"><Plus className="w-4 h-4" /> Créer mon premier album</Button>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-2">
      {albums.map((album, i) => (
        <AlbumListItem key={album.id} album={album} index={i} useAlbumMedia={useAlbumMedia}
          useAlbumShares={useAlbumShares} onClick={() => onSelect(album.id)} />
      ))}
    </div>
  );
};
...
const AlbumListItem = ({
  ...
  album: any; index: number; useAlbumMedia: (id: string) => any;
  useAlbumShares: (id: string) => any; onClick: () => void;
}) => {
  ...
  return (
    ...
    >
      {/* Cover thumbnail */}
      <div className="w-14 h-14 rounded-xl overflow-hidden bg-muted/60 flex-shrink-0 relative">
        {coverImage ? (
          <>
            {!loaded && (
            ...
            <img src={coverImage} alt={album.name} loading="lazy" onLoad={() => setLoaded(true)}
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-3 mt-1">
          <span className="text-[11px] text-muted-foreground">{media.length} {media.length !== 1 ? 'médias' : 'média'}</span>
          {shares.length > 0 && (
            <span className="text-[11px] text-muted-foreground flex items-center gap-0.5">
              <Users className="w-3 h-3" /> {shares.length} partage{shares.length !== 1 ? 's' : ''}
            </span>
          )}
        </div>
      </div>
...
const CreateAlbumView = ({
  ...
  name: string; description: string; onNameChange: (v: string) => void;
  onDescChange: (v: string) => void; onSubmit: () => void; isPending: boolean;
}) => (
  <div className="space-y-5 max-w-md mx-auto">
    ...
    <div className="space-y-3">
      <div>
        ...
        <Input placeholder="Ex: Mes photos préférées" value={name} onChange={(e) => onNameChange(e.target.value)} className="h-12 rounded-xl" autoFocus />
      </div>
      <div>
        <label className="text-sm font-medium mb-1.5 block">Description</label>
        <Textarea placeholder="Ajoute une description (optionnel)" value={description} onChange={(e) => onDescChange(e.target.value)} className="min-h-[80px] rounded-xl resize-none" />
      </div>
    </div>
    <Button className="w-full h-12 rounded-xl text-base font-semibold" onClick={onSubmit} disabled={isPending || !name.trim()}>
...
const AlbumDetailView = ({
  ...
  albumId: string; useAlbumMedia: (id: string) => any;
  onAddMedia: () => void; onDeleteMedia: (mediaId: string) => void;
  onMediaClick: (media: any[], index: number) => void;
}) => {
  const { data: rawMedia = [], isLoading } = useAlbumMedia(albumId);
  
  // Deduplicate media
  const media = rawMedia.filter((item: any, index: number, self: any[]) =>
  ...
  if (media.length === 0) {
    return (
      <div className="text-center py-16">
        ...
        <Button onClick={onAddMedia} className="rounded-xl gap-2"><ImagePlus className="w-4 h-4" /> Ajouter des médias</Button>
      </div>
    );
  }
      ...
      <div className="grid grid-cols-3 gap-1.5">
        {media.map((item: any, index: number) => (
          <div key={item.id} className="aspect-square rounded-xl overflow-hidden bg-muted relative group">
            <button onClick={() => onMediaClick(media, index)} className="w-full h-full">
              {item.media_type === 'image' ? (
                <img src={item.media_url} alt="" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-200" />
              ) : (
                <>
            ...
            <button
              ...
              className="absolute top-1.5 right-1.5 w-7 h-7 rounded-full bg-black/60 hover:bg-destructive text-white flex items-center justify-center shadow-lg transition-colors z-10"
            >
...
const SharesDialog = ({
  ...
  albumId: string; isOpen: boolean; onClose: () => void;
  useAlbumShares: (id: string) => any; stopSharing: any;
}) => {
      ...
      <DialogContent className="max-w-sm">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Share2 className="w-5 h-5 text-primary" /> Partages actifs
          </DialogTitle>
        </DialogHeader>
        {isLoading ? (
          ...
          <div className="space-y-2.5">
            {shares.map((share: any) => (
              <div key={share.id} className="flex items-center justify-between p-3 bg-muted/50 rounded-xl">
                <div>
                ...
                <Button size="sm" variant="destructive" onClick={() => stopSharing.mutateAsync(share.id)}
                  disabled={stopSharing.isPending} className="rounded-xl h-8 text-xs">
                  {stopSharing.isPending ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : (
                    <><StopCircle className="w-3.5 h-3.5 mr-1" /> Arrêter</>
                  )}
                </Button>
              </div>
            ))}
