...
// Batch signed URLs are now generated directly in the query below
  ...
  const { data: media = [], isLoading, error: mediaError } = useQuery({
    ...
    queryFn: async () => {
      ...
      const paths = data.map(item => {
        ...
        const publicMatch = path.match(/\/storage\/v1\/object\/public\/media\/(.+)$/);
        const authMatch = path.match(/\/storage\/v1\/object\/authenticated\/media\/(.+)$/);
        const signMatch = path.match(/\/storage\/v1\/object\/sign\/media\/(.+?)(\?|$)/);
        if (publicMatch) path = publicMatch[1];
        else if (authMatch) path = authMatch[1];
        else if (signMatch) path = signMatch[1];
        return path;
      });
      ...
      if (signError || !signedData) {
        ...
        return data.map(item => ({ ...item, signed_url: item.media_url }));
      }

      return data.map((item, i) => ({
        ...item,
        signed_url: signedData[i]?.signedUrl || item.media_url,
      }));
    },
