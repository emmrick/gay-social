...
const JoinedGroupsList = ({ onSelectGroup }: JoinedGroupsListProps) => {
  ...
  return (
    <>
      {/* Announcement Channel - always at top */}
      {announcementChannel && (
        <div className="px-4 pt-2 pb-1">
          <button
            onClick={() => onSelectGroup('announcement')}
            className={cn(
          ...
          >
            ...
            <div className="flex-1 min-w-0 text-left">
              <div className="flex items-center gap-2 mb-0.5">
                <h3 className="font-medium text-foreground truncate text-sm">Canal Informations</h3>
                <span className="text-[10px] px-1.5 py-0.5 bg-amber-500/20 text-amber-600 dark:text-amber-400 rounded-full font-medium flex-shrink-0">
                  Officiel
                </span>
              </div>
              <p className="text-xs text-muted-foreground">Annonces de l'équipe</p>
            </div>
