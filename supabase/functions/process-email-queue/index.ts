...
Deno.serve(async (req) => {
  ...
  for (const queue of ['auth_emails', 'transactional_emails']) {
    ...
    // Build retry count map from failed log entries
    ...
    for (let i = 0; i < messages.length; i++) {
      ...
      // Skip if already sent (dedup guard)
      ...
      try {
        ...
        if (result.status === 429) {
          ...
          await supabase.from('email_send_log').insert({
            ...
            recipient_email: payload.to,
            status: 'rate_limited',
            error_message: `Resend rate limit: ${result.body.slice(0, 500)}`,
          })
        ...
        if (result.status >= 400) {
          throw new Error(`Resend API error [${result.status}]: ${result.body.slice(0, 500)}`)
        }
