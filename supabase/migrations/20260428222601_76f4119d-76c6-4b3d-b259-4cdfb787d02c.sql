...
DECLARE
  ...
  -- Limite mensuelle : 5 jours offerts par mois × 5 cr = 25 cr / 30 jours
  v_max_monthly numeric := 25.0;
  v_top_up numeric;
...
BEGIN
  ...
  -- === Daily credits MONTHLY recharge logic (5 jours / mois max) ===
  -- On réutilise les colonnes weekly_* mais avec une fenêtre glissante de 30 jours
  IF v_credits.weekly_reset_date IS NULL OR v_credits.weekly_reset_date < CURRENT_DATE - INTERVAL '30 days' THEN
    UPDATE user_credits
  ...
  IF v_credits.daily_credits_last_reset IS NULL OR v_credits.daily_credits_last_reset < CURRENT_DATE THEN
    v_monthly_remaining := GREATEST(v_max_monthly - COALESCE(v_credits.weekly_credits_given, 0), 0);

    IF v_monthly_remaining > 0 AND v_credits.daily_credits < v_max_daily THEN
      v_top_up := v_max_daily - v_credits.daily_credits;
      v_top_up := LEAST(v_top_up, v_monthly_remaining);
  ...
  RETURN json_build_object(
    ...
    'monthly_credits_given', v_credits.weekly_credits_given,
    'monthly_credits_remaining', GREATEST(v_max_monthly - v_credits.weekly_credits_given, 0),
    'monthly_credits_max', v_max_monthly,
    'monthly_reset_date', v_credits.weekly_reset_date,
    -- Compat ascendante avec l'ancien front (clés weekly_*)
    'weekly_credits_given', v_credits.weekly_credits_given,
    'weekly_credits_remaining', GREATEST(v_max_monthly - v_credits.weekly_credits_given, 0),
